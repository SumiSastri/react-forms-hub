...
The function when run
      -  either sends the action and state tree data to the reducer to be transformed  
      -  or calls the reducer function
       -  When it calls the reducer function, it notifies component listeners to the  “next” change in the state tree
...
The store receives the map methods and via the dispatch(action) method converts the action payload to the dispatch payload
 Set up Dispatch changes to the components that have subscribed to the changes
    ...
    * function mapDispatchToProps(dispatch) - can send user data like admin fields, how toggles change, error handling
...
These dispatch functions are connected via the {connect} import to the counter component and exported back to the store
    ...
    * Provider
    * Wraps the redux component into the redux library framework
